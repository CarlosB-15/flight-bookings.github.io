...
if utente.lower() == 'user':
    ...
    giorno_partenza = int(input('In quale giorno (del mese corrente) desideri partire? '))
    giorno_arrivo = int(input('In quale giorno (del mese corrente) desideri arrivare? '))
    
    # caricamento soldi 
    saldo = int(input('Quanto desideri versare sul tuo conto? '))
    print('\nAttendere...')
    time.sleep(3)
    print(f'Saldo aggiornato con successo!\nSaldo disponibile: {saldo} euro.')
    
    # ricerca voli disponibili andata
    print(f'\nI voli disponibili il giorno {giorno_partenza} sono: ')
    ...
    if saldo < saldo_minimo[0]:
        print('\nSaldo insufficiente. Non è possibile prenotare alcun biglietto. Ricompilare il modulo ed aggiungere più soldi.')   
        sys.exit()
    ...
    if input('\nDesideri acquistare anche il volo del ritorno? si/no ') == 'si':
        ...
        if saldo < saldo_minimo[0]:
            ...
            saldo -= partenze.partenze[destinazione][partenza][giorno_arrivo][numero_biglietto-1][0] 
